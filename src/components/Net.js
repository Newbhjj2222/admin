'use client';

import React, { useState } from "react";
import Link from "next/link";
import { FiMenu, FiX } from "react-icons/fi"; // menu icons
import styles from "./Net.module.css";

const Net = () => {
  const [menuOpen, setMenuOpen] = useState(false);

  const toggleMenu = () => {
    setMenuOpen(!menuOpen);
  };

  return (
    <header className={styles.header}>
      <div className={styles.logo}>
        <Link href="/">NetBoard</Link>
      </div>

      <div className={styles.menuIcon} onClick={toggleMenu}>
        {menuOpen ? <FiX size={24} /> : <FiMenu size={24} />}
      </div>

      <nav className={`${styles.nav} ${menuOpen ? styles.open : ""}`}>
        <ul>
          <li><Link href="/">Home</Link></li>
          <li><Link href="https://admin-coral-five-19.vercel.app/Users">User management</Link></li>
           <li><Link href="https://admin-coral-five-19.vercel.app/Anslyser">Stories Checking</Link></li>
          <li><Link href="https://admin-coral-five-19.vercel.app/Bible">Send Bible verse</Link></li>
          <li><Link href="https://admin-coral-five-19.vercel.app/Manager">Web management</Link></li>
           <li><Link href="https://admin-coral-five-19.vercel.app/Push">Sell web</Link></li>
           <li><Link href="https://admin-coral-five-19.vercel.app/Nes">Depositers</Link></li>
           <li><Link href="https://admin-coral-five-19.vercel.app/Show">Tv show</Link></li>
          <li><Link href="https://admin-coral-five-19.vercel.app/Team">Authors Management</Link></li>
          <li><Link href="https://admin-coral-five-19.vercel.app/addTeam">Add Author</Link></li>
          <li><Link href="https://admin-coral-five-19.vercel.app/Request">Monetization Requests</Link></li>
          <li><Link href="https://admin-coral-five-19.vercel.app/monitize">Monitization management</Link></li>
          <li><Link href="https://admin-coral-five-19.vercel.app/rewards">Authors Balances</Link></li>
          <li><Link href="https://admin-coral-five-19.vercel.app/store">Store of Stories</Link></li>
          <li><Link href="https://admin-coral-five-19.vercel.app/verse">Bible verse manager</Link></li>
          <li><Link href="https://admin-coral-five-19.vercel.app/withdrawal">Withdrawal requests</Link></li>
          <li><Link href="https://admin-coral-five-19.vercel.app/Profile">Profile</Link></li>
          <li><Link href="https://dash-nine-rho.vercel.app/manage">News manager</Link></li>
          <li><Link href="https://dash-nine-rho.vercel.app/addnew">Publish news</Link></li>
          <li><Link href="https://dash-nine-rho.vercel.app/poly">Poly maker</Link></li>
        </ul>
      </nav>
    </header>
  );
};

export default Net;
